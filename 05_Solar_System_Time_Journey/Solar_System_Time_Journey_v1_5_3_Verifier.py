...
HTML = ROOT / "Solar_System_Time_Journey_v1_5_3.html"
...
add_tokens("identity", s, [
    "<title>Solar System Time Journey (SSTJ) v1.5.3</title>",
    ...
    'schema:"solar_system_time_journey_v1_5_3"',
...
checks["identity:html_filename"] = HTML.name == "Solar_System_Time_Journey_v1_5_3.html"
...
add_tokens("segment", s, [
    "FIRST_FIX","ACCEPTED","REJECTED_UNQUALIFIED_FIX_PAIR","REJECTED_STALE_TIMESTAMP","REJECTED_NONPOSITIVE_DT","REJECTED_INTERVAL_TOO_SHORT","ACCUMULATING_BELOW_POSITIONAL_NOISE","REJECTED_IMPLAUSIBLE_SPEED",
...
add_tokens("timestamp", s, [
    ...
    "sample.position_age_ms>POSITION_STALE_MAX_MS","journey.nonmonotonicTimestampCount++","callbackDt=(sample.ms-prev.ms)/1000","dt=(sample.ms-anchor.ms)/1000","if(!(dt>0))","if(callbackDt>90)journey.unclassifiedSeconds+=callbackDt"
])
...
add_tokens("pause", s, [
    ...
    "journey.lastSample=null","journey.distanceAnchorSample=null","journey.elevationAnchorAlt=null","journey.elevationAnchorAccuracy=null","releaseJourneyWakeLock()","startGpsWatch()","startDemoWalk()",'pause.textContent=journey.paused?"RESUME":"PAUSE"',
...
add_tokens("signal", s, [
    "function journeySignal()",'return"PAUSED"','return"SYNTHETIC"','return"SEALED"','return"EXCELLENT"','return"GOOD"','return"LIMITED"','return"WAITING"',"GPS_SIGNAL_LIMITED_AGE_MS=8000","GPS_SIGNAL_WAITING_AGE_MS=15000","age>=GPS_SIGNAL_WAITING_AGE_MS","age>=GPS_SIGNAL_LIMITED_AGE_MS","accuracy_m<=8","accuracy_m<=15","accuracy_m<=DISTANCE_ACCURACY_MAX_M","function journeySignalClass(signal)","jSignal"
])
...
add_tokens("acquisition", s, [
    "GPS_ASSIST_SILENCE_MS=12000","GPS_ASSIST_QUALITY_STALE_MS=12000","GPS_ASSIST_PROGRESS_STALE_MS=12000","GPS_ASSIST_MIN_INTERVAL_MS=12000",
    "GPS_ASSIST_TIMEOUT_MS=12000","GPS_ASSIST_WINDOW_MS=300000","GPS_ASSIST_MAX_REQUESTS_PER_WINDOW=20","GPS_FINAL_FIX_TIMEOUT_MS=6000",
    "function gpsAssistWindowCountAt(now,times)","function gpsAssistRateAvailableAt(now,times)","function gpsAssistShouldRequestAt(","function gpsQualityAssistShouldRequestAt(",
    "function gpsProgressAssistShouldRequestAt(","function scheduleGpsAcquisitionAssist(token)","async function requestGpsAcquisitionAssist(token)","gpsAssistRequestWallTimes",
    "gpsLastQualifiedCallbackWallMs","gpsLastAcceptedSegmentWallMs","gpsFinalFixRequestCount","gpsFinalFixSuccessCount","gpsFinalFixFailureCount"
])

add_tokens("selftest", s, [
    "async function runSSTJSelfTest()","Schema identity","GPS assist silence threshold","GPS assist rolling rate cap","GPS assist respects pause","GPS quality assist stale threshold","GPS quality assist cooldown","GPS progress assist stale threshold","GPS progress assist requires stalled state","Planetary lower guard","Planetary upper guard","WGS84 equatorial degree","WGS84 meridional degree","WGS84 direct-inverse closure","SHA-256 reference","Accuracy gate rejects poor fix","Poor fix adds no distance","No bridge across poor fix","Segment acceptance count","Raw coordinate path retained","Route directness unavailable across gap","Custom stride frozen","Breadcrumb hash chain root","Breadcrumb hash chain verification","Observed-derived separation","Measurement quality block","Digest reproducible","Accuracy-relative jitter rejection","Qualified sub-noise movement accumulates","Short interval rejection","Nonmonotonic timestamp rejection","Landing preference normalization","Distance unit normalization","Kilometre-mile conversion","Activity Solar Split defaults","Mile Solar Split defaults","Active duration excludes manual pause","Journey Signal excellent gate","Journey Signal ages to waiting","Journey Signal waits on unqualified fix","Solar Split threshold capture","Solar Split chain binding","Distance goal interpolated achievement","Goal and split record blocks","Unit metadata record block","Observed callback wall provenance","Pause-style anchor reset prevents bridge","Continuity event capture","Continuity record block","Ride steps unavailable","Reset split follows selected activity","Circular retention cap","Circular chronology","Circular reconciliation","SSTJ SELF-TEST:"
])
checks["selftest:exact_count_59"] = len(re.findall(r'check\("([^"]+)"', s)) == 59
checks["selftest:unique_names"] = len(set(re.findall(r'check\("([^"]+)"', s))) == 59
...
add_tokens("readme", readme, [
    "SOLAR SYSTEM TIME JOURNEY (SSTJ) v1.5.3","observer motion != change to planetary clock state","Automatic: Planet View on desktop, Journey View on mobile","Distance-unit preference","1 mi = 1.609344 km","canonical_distance_unit = km","distance_goal_km = distance_goal_display * 1.609344","active_time = wall_elapsed_time - manual_paused_time","manual_pause != pause of planetary state","Solar Splits","f = (D_target - D_before) / segment_distance","Journey Signal","callback_gap_ms > 15000","Recent Journeys","route coordinates","WGS84 ellipsoidal ground-distance calculation","WGS84_A_KM = 6378.137","WGS84_F = 1 / 298.257223563","MEAN_EARTH_RADIUS_KM = 6371.0088","reported_horizontal_accuracy_m <= 35","position_age_ms <= 90000","motion_signal_min_ratio = 0.5","raw_coordinate_path_km","ground_path_km","route_directness_ratio","Measurement-quality state","Observed and derived data","Browser capability variability","H_0 = SHA256(journey_chain_header)","H_n = SHA256(H_(n-1) || canonical_breadcrumb_n)","MAX_BREADCRUMBS = 5000","record SHA-256 != proof of presence","breadcrumb SHA-256 chain != proof of presence","await runSSTJSelfTest()","contains 59 checks","field-level validation","secure top-level HTTPS browser context","bounded location-acquisition assistance","freshness-aware Journey Signal","qualified sub-noise movement accumulation","ACCUMULATING_BELOW_POSITIONAL_NOISE","Field-level validation across multiple devices"
], ci=True)

add_tokens("boundary", boundary, [
    "SOLAR SYSTEM TIME JOURNEY (SSTJ) v1.5.3 - SCIENTIFIC BOUNDARY","observer motion -> changed planetary clock","Distance-unit preference","1 mi = 1.609344 km","canonical geometry and scientific record quantities remain metric","Manual Pause does not stop physical time","active_time = wall_elapsed_time - manual_paused_time","distance goals are declared in the unit frozen at Journey start","distance_goal_km = distance_goal_mi * 1.609344","Solar Splits","f = (D_target - D_before) / segment_distance","Journey Signal","callback_gap_ms > 15000","Recent Journey summaries exclude route coordinates and breadcrumb arrays","The browser API does not guarantee","WGS84 ellipsoidal Vincenty inverse","reported_horizontal_accuracy_m <= 35","position_age_ms <= 90000","motion_signal_min_ratio = 0.5","ACCUMULATING_BELOW_POSITIONAL_NOISE","REJECTED_IMPLAUSIBLE_SPEED","REJECTED_NONPOSITIVE_DT","REJECTED_INTERVAL_TOO_SHORT","raw_coordinate_path_km","route_directness_ratio","does not convert per-fix browser accuracy into a cumulative route-distance confidence interval","Observed values can include","Derived values can include","estimated_steps =","elevation_threshold_m =","H_0 = SHA256(journey_chain_header)","record SHA-256 != proof of presence","a navigation-certified geodesy product","a proof-of-presence system","local file is not treated as the primary live-geolocation deployment environment","reported horizontal accuracy and acquisition freshness","stable distance anchor for a bounded interval","bounded, rate-controlled fresh-position requests","Field-level validation across devices"
], ci=True)
...
rights = {
    ...
    "rights:third_party_version": "Solar System Time Journey (SSTJ) v1.5.3" in third_party,
}
checks.update(rights)

checks["hygiene:version_set"] = set(re.findall(r"v\d+\.\d+\.\d+", all_text)) <= {"v1.5.3"}
checks["hygiene:html_version_set"] = set(re.findall(r"v\d+\.\d+\.\d+", s)) <= {"v1.5.3"}
...
total = len(checks)
print("Solar System Time Journey (SSTJ) v1.5.3 verifier")
